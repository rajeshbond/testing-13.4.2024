      ...
      companyCells.forEach(cellType => {
        ...
        if (cellType === 'nsecode') {
          ...
          if (countValue === 1) {
            ...
            companyCell.style.backgroundColor = '#00C2CB'//'#6EACDA'; // Green for count > 1
...
async function init() {
  await fetchDataWithDelay('Advance', 'advance-table', 50, 'AdvanceData', 'AdvanceData');
  await fetchDataWithDelay('Bullish CrossOver', 'bullish-table', 50, 'BullishCrossOverData', 'BullishCrossOverData');
  await fetchDataWithDelay('Reversal Stock', 'reversal-1-table', 50, 'ReversalStockData', 'ReversalStockData');
  await fetchDataWithDelay('ActiveBy Volume', 'volume-table', 50, 'ActiveByVolumeData', 'ActiveByVolumeData');
  await fetchDataWithDelay('Range Breakout', 'range-break-table', 50, 'RangeBreakoutData', 'RangeBreakoutData');
